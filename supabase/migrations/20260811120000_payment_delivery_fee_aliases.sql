-- Keep storefront/Fyll Checkout delivery fee snapshots available for order creation.
-- This fixes rows where address/state already exist but delivery fee is still missing,
-- and accepts the wider alias set used by storefront/checkout payloads.
...
declare
  ...
  current_delivery_fee text := trim(coalesce(
    ...
    new.data->>'deliveryAmount',
    new.data->>'delivery_amount',
    new.data->>'shippingFee',
    new.data->>'shipping_fee',
    new.data->>'shippingCost',
    new.data->>'shipping_cost',
    new.data->>'shippingAmount',
    new.data->>'shipping_amount',
    new.data->>'shippingTotal',
    new.data->>'shipping_total',
    ''
  ));
begin
  ...
  if current_delivery_address <> '' and current_delivery_state <> '' and current_delivery_fee <> '' then
    return new;
  end if;
  ...
  if current_delivery_fee = '' and trim(coalesce(matched_order.data->>'deliveryFee', '')) <> '' then
    new.data := new.data || jsonb_build_object(
      ...
      'deliveryAmount', (matched_order.data->>'deliveryFee')::numeric,
      'delivery_amount', (matched_order.data->>'deliveryFee')::numeric,
      'shippingFee', (matched_order.data->>'deliveryFee')::numeric,
      'shipping_fee', (matched_order.data->>'deliveryFee')::numeric,
      'shippingAmount', (matched_order.data->>'deliveryFee')::numeric,
      'shipping_amount', (matched_order.data->>'deliveryFee')::numeric
    );
  end if;
...
set data = p.data
  || case
    when trim(coalesce(
        p.data->>'deliveryFee',
        p.data->>'delivery_fee',
        p.data->>'deliveryAmount',
        p.data->>'delivery_amount',
        p.data->>'shippingFee',
        p.data->>'shipping_fee',
        p.data->>'shippingCost',
        p.data->>'shipping_cost',
        p.data->>'shippingAmount',
        p.data->>'shipping_amount',
        p.data->>'shippingTotal',
        p.data->>'shipping_total',
        ''
      )) = ''
    ...
    then jsonb_build_object(
      ...
      'deliveryAmount', (matched.order_data->>'deliveryFee')::numeric,
      'delivery_amount', (matched.order_data->>'deliveryFee')::numeric,
      'shippingFee', (matched.order_data->>'deliveryFee')::numeric,
      'shipping_fee', (matched.order_data->>'deliveryFee')::numeric,
      'shippingAmount', (matched.order_data->>'deliveryFee')::numeric,
      'shipping_amount', (matched.order_data->>'deliveryFee')::numeric
    )
    else '{}'::jsonb
  end
from matched
